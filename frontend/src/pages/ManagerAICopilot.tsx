...
export function ManagerAICopilot() {
  ...
  useEffect(() => {
    document.title = 'AI Launch Copilot • BountyAI';
    return () => {
    ...
    <div className="flex h-full flex-1 flex-col">
      <GlassCard className="flex h-full flex-1 flex-col overflow-hidden">
        <header className="mb-4 flex items-center justify-between gap-4 border-b border-white/10 pb-4">
          <h1 className="text-2xl font-semibold text-foreground">S.T.E.V.E</h1>
          <select
            value={selectedProjectId ?? ''}
            onChange={(event) => setSelectedProjectId(event.target.value || null)}
            className="rounded-xl border border-black/10 bg-white/90 px-4 py-2 text-sm text-foreground focus:border-neon-teal focus:outline-none focus:ring-2 focus:ring-neon-teal/30"
          >
            ...
            {messages.map((message) => (
              <div
                ...
                className={cn(
                  ...
                  message.role === 'manager'
                    ? 'ml-auto bg-neon-teal/90 text-white'
              ...
              >
                <div className={cn(
                  ...
                  message.role === 'manager' ? 'text-white/80' : 'text-foreground/40'
                )}>
                  {message.role === 'manager' ? 'Manager' : 'S.T.E.V.E'}
                  {message.status === 'pending' ? ' • Thinking' : ''}
                </div>
                <p className={cn(
                  "mt-2 whitespace-pre-line leading-relaxed",
                  message.role === 'manager' ? 'text-white' : 'text-foreground'
                )}>{message.content}</p>
        ...
        <footer className="mt-4 space-y-3 border-t border-white/10 pt-4">
            {error && (
              <div className="rounded-2xl border border-red-300 bg-red-100 px-4 py-2 text-xs font-semibold text-red-800">
                {error}
              </div>
            )}

            {uploadError && (
              <div className="rounded-2xl border border-neon-orange/40 bg-neon-orange/10 px-4 py-2 text-xs font-semibold text-neon-orange">
                {uploadError}
              </div>
            )}

            {speechError && !error && (
              <div className="rounded-2xl border border-neon-purple/40 bg-neon-purple/10 px-4 py-2 text-xs font-semibold text-neon-purple">
                {speechError}
              </div>
            )}
            ...
            {attachments.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {attachments.map((file) => (
                  <div
                    key={file.id}
                    className="flex items-center gap-2 rounded-xl border border-foreground/10 bg-white/80 px-3 py-1 text-xs text-foreground/60"
                  >
                    <span>{file.name}</span>
            ...
            {interimTranscript && isRecording && (
              <div className="rounded-2xl border border-neon-teal/30 bg-neon-teal/10 px-4 py-2 text-xs text-neon-teal">
                  ...
                  <div className="flex-1">
                    ...
                    {!lastRecommendation.teamId && (
                      <div className="mt-2 text-xs text-red-600">
                        Warning: Team ID missing. Ask AI to provide specific team recommendation.
                      </div>
                    )}
            ...
            <div className="relative">
              <textarea
                ...
                className="w-full resize-none rounded-3xl border border-black/10 bg-white/90 px-5 py-4 pr-24 text-sm text-foreground placeholder:text-foreground/40 focus:border-neon-teal focus:outline-none focus:ring-2 focus:ring-neon-teal/30"
                rows={3}
              />
              
              {/* Input Controls */}
              <div className="absolute bottom-4 right-4 flex items-center gap-2">
                <label className="cursor-pointer rounded-full p-2 text-foreground/40 transition hover:bg-foreground/5 hover:text-foreground">
                ...
                <button
                  ...
                  className="rounded-full bg-neon-teal p-2 text-white transition-all hover:bg-neon-teal/90 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
